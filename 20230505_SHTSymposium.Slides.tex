...
\newcommand{\everyframe}{10}% use only every nth frame for the animations
...
\section{Histologie \(\leftrightarrow\) \uct}
\begin{frame}
 	...
 	\begin{columns}%
		...
		\end{column}%
 		\begin{column}{0.5\linewidth}%
 			\centering
 			\uct Pro\only<2>{/\color{ubRed}{Contra}}
			\begin{itemize}
				\item Zerstörungsfrei, virtuelle Schnitte in \emph{jeder} Richtung, komplett 3D
				\item \emph{Richtige} Histologie danach immer noch möglich
				\item Histologische Analysemethoden (z.B. Stereologie) mit Tomographie\-daten
				\item<2-> \color{ubRed}{Differenzierung \emph{nur} über Röntgen\-dichte}				
				\item<2-> \color{ubRed}{Kosten}
				\item<2-> \color{ubRed}{\emph{Grosse} Datenmengen}
			\end{itemize}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}%
		\begin{column}{0.35\linewidth}%
			\only<2-6|handout:1>{%
				...
				\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
					...
					\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+333.33,\y) node [midway,above] {\shadowtext{\SI{5}{\milli\meter}}};
				\only<3|handout:1>{%
		...
		\begin{column}{0.35\linewidth}%
			...
			\only<7|handout:0>{%
				\mode<beamer>{\animategraphics[loop,autoplay,width=\imagewidth,every=\everyframe]{24}{./images/Mouse01_Lugol_15pct_20230421/animation_frames/frame0}{000}{080}}%
				\mode<handout>{\includegraphics[width=\imagewidth]{./images/Mouse01_Lugol_15pct_20230421/animation_frames/frame0019}}%
			}%
		\end{column}%
		\begin{column}{0.35\linewidth}%
			\only<4-6|handout:1>{%
...
\begin{frame}
	...
	\begin{columns}%
		...
		\begin{column}{0.35\linewidth}%
			...
			\only<3|handout:0>{%
				\animategraphics[width=\imagewidth,every=\everyframe]{24}{./images/Mouse01_Lugol_15pct_20230421/animation_frames/frame0}{241}{480}%rotate once, show crop of back muscles
				}%
			\only<4|handout:0>{%
				\animategraphics[width=\imagewidth,every=\everyframe]{24}{./images/Mouse01_Lugol_15pct_20230421/animation_frames/frame0}{481}{720}%go back to start
				}%
		\end{column}%
		\begin{column}{0.35\linewidth}%
			\only<1|handout:1>{%
...
\begin{frame}[label=current]
		...
		\begin{columns}%
			\begin{column}{0.5\linewidth}%
				...
				\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
					...
					\spy [black] on (827,1241) in node at (2576,966) [anchor=east];
			...
			\begin{column}{0.5\linewidth}%
				...
				\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
					...
					\spy [ultra thick,white] on (2296,759) in node at (0,572.5) [anchor=west];
	...
	\only<2>{%
			\centering%
			\mode<beamer>{%
				\animategraphics[loop,autoplay,width=\imagewidth,every=\everyframe]{24}{./images/R108/2010a/mrg/R108C04At-mrg/animation_frames/frame0}{000}{200}%
			}%
			\mode<handout>{%
	...
	\only<3>{%
		\centering%
		\animategraphics[loop,autoplay,width=\imagewidth,every=\everyframe]{24}{./images/R108/2010a/mrg/R108C04At-mrg/animation_frames/frame0}{201}{420}%
			};%
	\only<4>{%
		\centering%
		\animategraphics[loop,autoplay,width=\imagewidth,every=\everyframe]{24}{./images/R108/2010a/mrg/R108C04At-mrg/animation_frames/frame0}{421}{677}%
	}%
	\only<4>{%
		\centering%
		\animategraphics[loop,autoplay,width=\imagewidth,every=\everyframe]{24}{./images/R108/2010a/mrg/R108C04At-mrg/animation_frames/frame0}{678}{900}%
	}%
\end{frame}
...
\begin{frame}%
	...
	\begin{columns}%
		\begin{column}{0.5\linewidth}%
			\centering%
			\includegraphics[height=\imageheight]{./images/DigitalPathology/Amjad_B08_10543_III_F_HE_7.9x.png}%
		...
		\begin{column}{0.5\linewidth}%
			...
			\begin{tikzpicture}[x=\imagescale,y=-\imagescale]
				\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[height=\imageheight]{./images/DigitalPathology/10543_1mm_Vert_04um_CCD_nof_60kV/rec/10543_1mm_Vert_04um_CCD_nof_60kV_rec00001241}};
				...
				\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+625,\y) node [midway,above] {\shadowtext{\SI{250}{\micro\meter}}};
			\end{tikzpicture}%
		\end{column}
	\end{columns}%
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item<1-> \uct macht effiziente und zerstörungsfreie Bildgebung in 3D
		\item<1-> Auflösung von \uct-Daten erreichen histologische Standardmethoden
		\item<1-> Klassische Färbung ist \emph{nicht} möglich, je nachdem aber auch nicht nötig
