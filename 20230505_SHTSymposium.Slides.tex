...
\begin{frame}
	\frametitle{Wiso braucht es überhaupt \uct-Bildgebung?}
	\begin{columns}
		\begin{column}{0.49\linewidth}
...
% 					\item asdf
% 				\end{itemize}}
% 				{\color{ubRedComplementary1}\begin{itemize}
% 					\item asdf
% 					\item asdf
% 				\end{itemize}}				
% 				{\color{ubRedComplementary2}\begin{itemize}
% 					\item asdf
% 					\item asdf
% 				\end{itemize}}								
 			\end{column}
...
% 					\item[-] Schnitte durch Samples sind \emph{nicht} virtuell
% 				\begin{end}
 			\end{column}
 		\end{columns}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.43\linewidth}
			...
			\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
			\begin{scope}
				\clip (0,0) rectangle (1944,1944);
				%\clip (972.0,972.0) circle (972.0);
				\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./images/Mouse01_Lugol_15pct_20230421.rec.Middle.Axial}};
			\end{scope}
			%\spy [red] on (1644,1644) in node at (0,0) [anchor=north west];
			...
			\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+33.333,\y) node [midway,above] {\shadowtext{\SI{500}{\micro\meter}}};
			%\draw[color=red, anchor=south west] (0,1944) node [fill=white, semitransparent] {Legend} node {Legend};
		\end{tikzpicture}%
		\end{columns}
		\begin{column}{0.43\linewidth}
			\pgfmathsetlength{\imagescale}{\imagewidth/1944}%
...
\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
	\begin{scope}
		\clip (0,0) rectangle (1944,1941);
		%\clip (972.0,970.5) circle (970.5);
		\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./images/Mouse01_Lugol_15pct_20230421.rec.Middle.Sagittal}};
	\end{scope}
	%\spy [red] on (1644,1641) in node at (0,0) [anchor=north west];
	% 1944.000px = 29.16mm -> 100px = 1500.000um -> 33.333px = 500um, 6.667px = 100um
	...
	\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+33.333,\y) node [midway,above] {\shadowtext{\SI{500}{\micro\meter}}};
	%\draw[color=red, anchor=south west] (0,1941) node [fill=white, semitransparent] {Legend} node {Legend};
\end{tikzpicture}%
		\end{columns}
		\begin{column}{0.43\linewidth}
			\pgfmathsetlength{\imagescale}{\imagewidth/1944}%
...
\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
	\begin{scope}
		\clip (0,0) rectangle (1944,1941);
		%\clip (972.0,970.5) circle (970.5);
		\node[anchor=north west, inner sep=0pt, outer sep=0pt] at (0,0) {\includegraphics[width=\imagewidth]{./images/Mouse01_Lugol_15pct_20230421.rec.Middle.Coronal}};
	\end{scope}
	%\spy [red] on (1644,1641) in node at (0,0) [anchor=north west];
	% 1944.000px = 29.16mm -> 100px = 1500.000um -> 33.333px = 500um, 6.667px = 100um
	%\draw[|-|,blue,thick] (0,970) -- (1944,970) node [sloped,midway,above,fill=white,semitransparent,text opacity=1] {\SI{29.16}{\milli\meter} (1944px) TEMPORARY!};
 ...
 \begin{frame}[label=current]
 		...
 		\begin{columns}
 			\begin{column}{0.49\linewidth}
 				...
 				\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
 					...
 					\only<2|handout:0>{\spy [black] on (827,1241) in node at (2576,966) [anchor=east];}
 			...
 			\begin{column}{0.49\linewidth}
 				...
 				\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
 					...
 					\only<2|handout:0>{\spy [white] on (2296,759) in node at (0,572.5) [anchor=west];}
	...
	\only<3|handout:2>{%
		\begin{tikzpicture}[remember picture,overlay]%
			\node at (current page.center){%	
				\mode<beamer>{%
					\animategraphics[loop,autoplay,width=\linewidth,every=\everyframe]{24}{.images/R108/2010a/mrg/R108C04At-mrg/animation_frames/R108C04At-mrg_crop_rotate_Composite_tif0}{000}{180}%
					}%
				\mode<handout>{%
					\includegraphics[width=\linewidth]{.images/R108/2010a/mrg/R108C04At-mrg/animation_frames/R108C04At-mrg_crop_rotate_Composite_tif0060}%
				}
			};%
		\end{tikzpicture}%
	}%
...
\begin{frame}
	...
	\begin{tikzpicture}[x=\imagescale,y=-\imagescale,spy using outlines={rectangle,magnification=\mag,size=\size,connect spies}]
		\begin{scope}
			%\clip (0,0) rectangle (3296,3296);
			\clip (1648.0,1648.0) circle (1648.0);
		...
		\end{scope}
		%\spy [red] on (2996,2996) in node at (0,0) [anchor=north west];
		...
		\draw[|-|,white,thick,shadowed] (\x,\y) -- (\x+1250.000,\y) node [midway,above] {\shadowtext{\SI{500}{\micro\meter}}};
		%\draw[color=red, anchor=south west] (0,3296) node [fill=white, semitransparent] {Legend} node {Legend};
	\end{tikzpicture}%
\end{frame}
